...
| A student I accepted/rejected is back on Sheet 1 | v2.7.5+: this means a NEW *different* mismatch type arose for that student. Look at the Mismatch Summary column — it'll show something different from what you previously actioned (e.g. you fixed "Grade" before, now it shows "Grade, Email" because an Email mismatch surfaced). Re-action the new mismatch. If you actually want to re-action the OLD mismatch (e.g. you accidentally rejected and want to accept), open the hidden `_RejectedData` / `_ApprovedData` / etc tab via Apps Script editor and delete that student's row — the next pipeline run will re-surface them on Sheet 1. |
