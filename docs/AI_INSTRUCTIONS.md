...
## Summer School Columns (v2.9.0; AFMS v2.9.1; JRES v2.9.2; AFES v2.9.3)

Summer school enrollment is tracked with 5 columns appended (after Unenroll) at each layer of the chain, for the 6 summer-school campuses: JHMS, JHES, JRHS, JRES (Jasper) and AFMS, AFES (Allendale). Provisioned by `setup_summer_school_columns.py` (idempotent, find-or-append by header so column positions adapt per campus), which mirrors the Unenroll precedent:
...
Data flow is identical to Unenroll: `SR (typed) -> MR (arrayformula) -> CMR (importrange)`. The grade column is set to plain NUMBER format (appended cells otherwise inherit a date format and show grades as `01/07/1900`). Subjects: "Language and Fast Math" for the 4 Jasper schools (JHMS/JHES/JRHS/JRES); per-student "Language" or "Math" for AFMS; "Language and Math" for AFES. Column POSITIONS vary by campus (find-or-append, never hardcoded): Jasper (39-col CMR) at SR AC..AG / MR+CMR AE..AI; AFMS (29-col CMR) at SR AB..AF / MR AC..AG / CMR AD..AH.

**Combined view (v2.9.1)**: `build_summer_roster_tab()` (re)builds the `Summer School Roster` tab on the CMR: one live QUERY over all `SUMMER_TABS`, normalized to core A:N + the 5 summer columns. Each school's summer block is brought to a fixed output position via a per-school `{core, summer}` horizontal join, so the flag is always output Col15 and ONE QUERY (`where Col15 = true`) filters across all schools regardless of their differing absolute summer-column positions. Re-run `setup_summer_school_columns.py` to refresh it after the school list changes (416 rows across 6 schools as of v2.9.3).

Per-student VALUES are NOT in the repo: the provided summer rosters are student PII, loaded once via a gitignored `_scratch_*` script (deleted after the run). That loader fuzzy-matches the name-only lists to existing SR rows (clean fuzzy-subset with a 3+token fallback and a v2.9.2 lenient short-token tier for misses, e.g. Ava->Avah, uniqueness-gated) and writes only confident unique matches; ambiguous/unmatched names are reported for manual review, never guessed, and no new roster rows are created. To re-load or extend, re-provision then re-run a fresh scratch loader. AFES (v2.9.3) is the exception to name-list matching: its loader is RULE-based, flagging every AFES Student Roster row with Admission Status = "Enrolled" (current grade, "Language and Math", blank teacher), idempotent so a re-run tracks enrollment.
