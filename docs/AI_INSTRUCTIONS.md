...
| `summer_roster_diff.py` | ~230 | v2.9.8: diff the combined Summer School Roster vs a past Drive revision (`--since-rev`/`--since-date`, optional `--csv`) or vs the source of truth (`--vs-source`, staleness detector, exit 1 on drift); `--mark-red` appends new students to `_Highlight`. PII-free |
...
## Summer School Columns (v2.9.0-2.9.12; 6 schools, SR-checkbox driven)
...
- **`_SummerList`** (hidden, per ISR): columns [email, grade, subjects, teacher_email, teacher]. An optional email-keyed bulk/programmatic override OR'd into the MR flag. EMPTY by default since v2.9.12 (its 499 students were backfilled into the SR checkboxes, then it was cleared); use it only for bulk adds you would rather not check by hand.
- **MR** "MAP Roster" summer columns (v2.9.11): each column COALESCES the row's own SR cell with the email-keyed `_SummerList` lookup. flag = `(SR 'Summer School' checkbox = TRUE) OR ISNUMBER(MATCH(B2:B,'_SummerList'!A:A))`; each detail = the SR cell if filled, else `VLOOKUP` into `_SummerList`. Both sources are sort-safe.
- **SR** "Student Roster" summer columns (v2.9.12): the "Summer School" column is an editable BOOLEAN checkbox and the PRIMARY source of truth. All 499 current summer students are SR-checked (backfilled v2.9.12) with grade/subjects/teacher/teacher_email in the adjacent SR summer cells. It sits INSIDE each school's sortable Table (verified all 6: the summer cols are within the Table range), so a checked box travels with its row on sort. A manager checks the box to ADD a student and UNCHECKS to REMOVE (with `_SummerList` empty, nothing else keeps them flagged).
- **CMR** campus-tab summer cols: `=IMPORTRANGE(ISR,"MAP Roster!<col>2:<col>")` (unchanged; same `Allow access` note as Unenroll).

Why email-keyed: the SR re-sorts (sticky-sorted Table), which detached the old STATIC row-keyed flags onto the wrong students (the JRES incident, CHANGELOG v2.9.5). Keying each MR row to its OWN email means re-sorting recomputes correctly per row, so the flag can never misalign again. Email (not student_id) because it is universal: it also covers "email-only" students whose Student ID is still blank. NOTE: `provision_mr` clears the summer-column region before writing the spilling ARRAYFORMULA (stale static values, e.g. a `False` block from the old mirror, otherwise block the spill with `#REF!`). v2.9.11 RESTORED the SR checkbox as an additional, sort-safe input (OR'd into the MR coalesce): because the summer cols are inside each school's Table, a checked box sorts WITH its row (avoiding the original out-of-Table detach that caused the JRES incident); the email-keyed `_SummerList` remains for bulk/programmatic adds. So a student is flagged if the SR box is checked OR their email is in `_SummerList`. v2.9.12 BACKFILLED the SR checkbox for all 499 currently-flagged students (copying grade/subjects/teacher from `_SummerList`, matched by email) then EMPTIED `_SummerList`, making the SR checkbox the SOLE working source: check = add, uncheck = remove, feeding MR -> CMR -> combined roster live. Verified by a staged backfill (MR-TRUE stayed 499 after the `_SummerList` clear) + a live uncheck test (uncheck -> MR FALSE -> re-check -> TRUE).
...
**Tool: `summer_roster_diff.py` (v2.9.8).** Packages the above. `--since-rev ID` / `--since-date YYYY-MM-DD` diff the live roster vs that Drive revision (added/removed by campus+name+grade, optional `--csv`); `--vs-source` compares the visible roster vs the source of truth (union of each ISR's MAP Roster computed-TRUE emails) and exits 1 on drift (run this BEFORE the morning configuration run to catch a stale per-campus IMPORTRANGE before students are configured); `--mark-red` appends added-not-already-red emails to `_Highlight`. The 2026-06-03 incident: the JRES per-campus CMR `=IMPORTRANGE(ISR,"MAP Roster!AE2:AE")` was stale (showed the old incident students while the ISR MR was correct), so the morning roster was wrong. On detected drift, fix by re-running `setup_summer_school_columns.py` (rebuilds the QUERY + re-writes the per-campus IMPORTRANGE to force a fresh pull).
