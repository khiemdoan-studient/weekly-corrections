...
## Summer School Columns (v2.9.0; AFMS v2.9.1; JRES v2.9.2)

Summer school enrollment is tracked with 5 columns appended (after Unenroll) at each layer of the chain, for the 5 summer-school campuses: JHMS, JHES, JRHS, JRES (Jasper) and AFMS (Allendale). Provisioned by `setup_summer_school_columns.py` (idempotent, find-or-append by header so column positions adapt per campus), which mirrors the Unenroll precedent:
